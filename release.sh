...
echo '[2/13] Syntax + unit/security (isolated state)'; node --check server.mjs; node --check mcp.mjs; node test.mjs; node reset-test.mjs; node security-regression-test.mjs; node launchagent-test.mjs
...
echo '[6/13] DevSpace compatibility'; node compat-test.mjs
echo '[7/13] Custom GPT Action bridge'; node action-test.mjs
echo '[8/13] Privacy'; node privacy-check.mjs
echo '[9/13] Render UI (Vite + React + Tailwind v4)'; (cd ui && npm install --silent && npm run build)
echo '[10/13] Execution E2E (universal helper)'; node e2e.mjs
