...
- Run `node test.mjs` for core/MCP/security tests.
- Universal release binaries (arm64 + x86_64) are produced by `./release.sh`. To build manually, build each architecture with `swift build -c release --arch arm64 --scratch-path .build/arm64` (and the same with `x86_64`) and combine with `lipo -create`.
