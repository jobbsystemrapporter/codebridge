...
export function classifyExec(executable,args=[]){if(executable.includes('/'))return 'blocked';const name=path.basename(executable);if(NEVER.has(name))return 'blocked';const r=RULES[name];if(!r)return 'approval';if(r.destructive?.(args))return 'destructive';if(r.safe?.(args))return 'safe';return 'approval'}
